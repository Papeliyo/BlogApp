package com.papeliyodev.blogapp.data.remote.auth
...
import com.papeliyodev.blogapp.data.model.User
...
class AuthDataSource {
    ...
    suspend fun singUp(email: String, password: String, username: String): FirebaseUser? {
        ...
        authResult.user?.uid?.let { uid->
            FirebaseFirestore.getInstance().collection("users").document(uid).set(User(email,username, photo_url = "FOTO URL.PMG")).await()
        }
        return authResult.user
    }
